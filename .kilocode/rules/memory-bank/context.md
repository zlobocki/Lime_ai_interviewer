...
| 2026-02-26 | Fix v1.7.0: CSRF token verification failing (400 error) — Yii's CSRF check runs before newDirectRequest fires and rejects JSON POST bodies because YII_CSRF_TOKEN is not present as a form field. Fixed by subscribing to beforeControllerAction and disabling CSRF validation (enableCsrfValidation=false) for requests targeting plugin=AIInterview on the plugins/direct route. Our own security (survey session check + admin permission check) provides equivalent protection. Bumped to v1.7.0. Repackaged AIInterview.zip. |
| 2026-02-27 | Fix v1.8.0: CSRF 400 still occurring — beforeControllerAction approach did not work (Yii CSRF check fires before the event or enableCsrfValidation=false is not respected). Root fix: changed JS to send POST as application/x-www-form-urlencoded with YII_CSRF_TOKEN as a form field and JSON data as a 'payload' field (JS v1.5.0). Updated PHP handleChatRequest() to read from $_POST['payload'] with fallback to php://input. Removed beforeControllerAction subscription. Bumped to v1.8.0. Repackaged AIInterview.zip. |
| 2026-02-27 | Fix v1.9.0: Question completely invisible in live survey — afterRenderQuestion was corrupting the Twig-rendered widget HTML. In live survey mode, LimeSurvey uses answer.twig to render the widget; afterRenderQuestion then fired and its regex matched the hidden answer textarea INSIDE the already-rendered widget, replacing it with the full widget HTML again (nested widget corruption). Fixed by: (1) adding a strpos() check in afterRenderQuestion — if the widget div id is already present in the HTML, skip the replacement (assets are still registered); (2) fixed answer.twig hidden textarea from display:none to position:absolute/opacity:0 (same fix as v1.5.0 for PHP-generated HTML, needed for mandatory-field validation). Bumped to v1.9.0. Repackaged AIInterview.zip. |
| 2026-02-27 | Fix v1.10.0: Question still not displaying in live survey — v1.9.0 fix (Twig renders widget, afterRenderQuestion skips) left the widget invisible because the Twig template was rendering the widget but something prevented it from displaying. Root fix: rewrote answer.twig to render only a plain textarea (delegating widget injection entirely to afterRenderQuestion). Fixed afterRenderQuestion regex to use negative lookahead excluding class="ai-interview-answer-field" so it cannot match the hidden answer textarea inside the widget. Removed the overly broad class="ai-interview-widget guard check (kept only the specific widget ID check). Bumped to v1.10.0. Repackaged AIInterview.zip. |
| 2026-02-28 | Fix v1.11.0: Answer not saved after completing survey — hidden answer textarea was missing the "ls-answers" CSS class. LimeSurvey's survey JavaScript uses $(".ls-answers") to collect form field values for AJAX submission; without this class the transcript was never included in the POST data and the answer column stayed empty/NULL. Added "ls-answers" to the hidden textarea's class list in buildWidgetHtml(). Bumped to v1.11.0. Repackaged AIInterview.zip. |
